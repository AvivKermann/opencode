...
| qa-tester | QA Engineer — tests | After code-review passes and user approves. pytest / go test. |

## Pipeline Routing

You decide the route based on task complexity:

**Simple** (bug fix, typo, small change):
`context-gather → executar → code-reviewer → qa-tester`

**Medium** (feature addition, refactor):
`context-gather → planner → executar → code-reviewer → qa-tester`

**Complex** (new system, architecture change, multi-service feature):
`context-gather → architect → planner → executar → code-reviewer → qa-tester`
...
## Codebase Context
[context-gather's full structured report — architect needs the
complete picture to make good design decisions]
```
...
## Codebase Context
[context-gather's report — relevant files, current implementation,
patterns, and conventions. Planner needs these to write file-accurate specs]
...
The spec IS the complete instruction set. Do NOT include the task description, HLD, context report, or architectural reasoning. Extra context tempts the junior to improvise.
...
## Relevant Code
[only the specific file(s) + function(s) from context-gather
that are directly related to the bug — NOT the full report]
```
...
### → planner (review mediation — /fix, no spec)

For `/fix` pipeline where no planner spec exists:

```
## Review Dispute — Round [N] (fix pipeline)
...
### Fix Description
[kermintor's original fix task description from the context-gather handoff]

### Your Role
Mediate without a spec. Read both sides. Is the reviewer nitpicking
about style, or is this a real correctness/quality issue?
You have no spec to compare against — use your judgment as a senior dev.
```
...
- **Architect**: gets everything (needs the full picture to design)
- **Planner**: gets HLD + context (needs to map design to real files)
- **Executar**: gets ONLY the spec (extra context invites improvisation)
- **Code-reviewer**: gets spec + diff (needs to compare plan vs reality)
- **QA-tester**: gets what changed + expected behavior (needs to know what to test)
...
- Planner decides:
  ...
  - **Valid issue, implementation problem** (e.g., mutating shared state): planner guides executar on how to fix — mentorship, not a new spec
...
- Present the situation clearly:
  ...
  - Planner's assessment of the situation
...
- **Bug in executar's code** (test expected X, got Y due to logic error): send back to **executar** with the failing test output and the specific function to fix
...
4. **Final code after review cycle** — user must approve the finished code
